...
- Made ERC-7572 `contractURI()` metadata required for application conformance, with nonempty name and description plus queries, actions, or both. Added `spec/METADATA.md`, inherited `IERC7572` in both application interfaces, and aligned the ERC working paper. Earlier deployments without metadata need upgrading or redeployment.
- Added immutable inline JSON metadata to all seven reference adapters and the local fixture. Runtime sizes remain below EIP-170; Avantis is the largest at 23,123 bytes (1,453 bytes remaining).
- Added matching TypeScript and bundled Python metadata validation/resolution, shared data-URI vectors, and complete chain/address discovery coverage. Both clients accept inline UTF-8, percent-encoded, and Base64 JSON, enforce a 64 KiB document limit, and support authorized HTTPS retrieval or IPFS through a configured HTTPS gateway. Required metadata failures are explicit; catalog names never substitute for contract self-description.
- Updated the skill discovery workflow and console to display validated contract-provided name/description and URI provenance. The console pins discovery to one block and exposes metadata HTTP settings before loading. The Pages build automatically packages the canonical skill changes.
- Deployed the metadata-capable Base replacements and verified all four adapters plus the Avantis descriptor companion on BaseScan with solc 0.8.30 and optimizer 200. Current adapters: Aerodrome `0x1b64ccafc9669dc920bee37fffb999053ab6bee0`, Moonwell `0xd8e7a909318c36bc17a5eee6f2f8f44c848f29f3`, Avantis `0x300030fea92f4281894aefde5f2261fe12c0afdb`, Relay `0x4ab46c803b53ef51e9813c512de7ccef6214ea92`. The descriptor companion is `0x3CAe35596c22E6Be89109E0daa4D3032fC56B92d`. Deployment transactions and constructor configuration are recorded in the adapter documents. Updated the catalog and console examples to these addresses.
- Verified live chain/address discovery through the Python client and browser console for all four replacements. Validation passed: 40 Forge tests, 72 Bun tests (including all seven adapters through both clients on Anvil), 13 Python tests, formatter/linter/typechecks, and the web build.
- Published the console, updated catalog, and skill to Cloudflare Pages: `https://7f63d82b.onchain-app-interfaces.pages.dev` (production alias `https://onchain-app-interfaces.pages.dev/`). Verified production browser discovery for all four examples, the served catalog against its source, and all 14 published skill files against the canonical skill directory. Published skill version: `6c7bdc1a6a448c1f`.
- Added a curated verified-interface manifest at `web/public/interfaces.json`, served from the Pages site as `interfaces.json`. It lists the deployed Base reference adapters (Aerodrome, Moonwell, Avantis, Relay) with chain ID, address, display name, category, keywords, and a `verified` flag. Consumers use it to mark a matching registered adapter as verified and to offer the entries for registration; registration does not itself authenticate an adapter. The `bytecodeHash` field is optional and currently omitted.
